...
$publisherLogo = 'https://grootacademy.com/wp-content/uploads/2024/12/cropped-Groot-Logo-transparent.png';
...
$resolveAbsoluteUrl = static function ($value) use ($siteUrl, $canonical) {
    ...
    if (substr($value, 0, 1) === '/') {
        return rtrim($siteUrl, '/') . $value;
    }
    if ($canonical) {
        return rtrim((string) $canonical, '/') . '/' . ltrim($value, '/');
    }
...
<head>
    ...
    <?php if ($featuredImageUrl && $featuredImageAlt): ?><meta property="og:image:alt" content="<?= $esc($featuredImageAlt) ?>"><?php endif; ?>
    ...
    <?php if ($featuredImageUrl && $featuredImageAlt): ?><meta name="twitter:image:alt" content="<?= $esc($featuredImageAlt) ?>"><?php endif; ?>
